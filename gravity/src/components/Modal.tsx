...
export const Modal: React.FC<IModalProps> = ({ isOpen, onClose, children, className, modal, size = "lg" }) => {
    return (
                        ...
                        className={clsx(
                            {
                                ...
                                "tablet:w-[80vw] tablet:h-[70%] tablet:max-w-[800px] tablet:max-h-[700px]":
                                    size === "xl",
                                "min-h-[200px] max-h-[80vh]": size !== "full",
                            },
...
export interface IModalHeaderProps extends React.BaseHTMLAttributes<HTMLDivElement> {
    className?: string;
    title: string;
    subtitle?: string;
...
export const ModalHeader: React.FC<IModalHeaderProps> = ({
    ...
    return (
        <div className={twMerge("relative w-full px-6 py-4 border-b border-slate-200 bg-white", className)}>
            <div className={clsx("basis-full flex flex-col gap-1", { "mt-20": icon, "mt-0": !icon })}>
                <DialogTitle className={"text-lg font-semibold text-slate-900 z-10"} ref={ref}>
                    {title}
                </DialogTitle>
            ...
            {icon && (
                <>
                    <div
                        className={clsx("absolute top-0 left-0 mt-4 ml-4 p-2 rounded-full", {
...
export interface IModalActionsProps extends React.BaseHTMLAttributes<HTMLDivElement> {
    children?: React.ReactNode;
}

export const ModalActions: React.FC<IModalActionsProps> = ({ className, children }) => {
    return <div className={twMerge("w-full mt-auto flex gap-3 p-4 bg-white", className)}>{children}</div>;
};

export interface IModalContentProps extends React.BaseHTMLAttributes<HTMLDivElement> {
    className?: string;
    children?: React.ReactNode;
}

export const ModalContent: React.FC<IModalContentProps> = ({ className, children }) => {
    const contentRef = useRef<HTMLDivElement>(null);
    const [hasScrollbar, setHasScrollbar] = useState(false);

    const checkForScrollbar = useCallback(() => {
        if (contentRef.current) {
            const hasVerticalScrollbar = contentRef.current.scrollHeight > contentRef.current.clientHeight;
